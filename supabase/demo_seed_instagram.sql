-- SalonFlow Instagram demo dataset
...
-- If you have more than one active organization, set target_slug below.
...
begin
  ...
  if target_slug is not null then
    ...
    raise exception 'Found % active organizations. Set target_slug at the top of supabase/demo_seed_instagram.sql.', org_count;
  end if;
  ...
  for i in 1..array_length(names,1) loop
    ...
    if c_id is null then
      insert into public.clients (
        organization_id,first_name,last_name,email,phone,date_of_birth,notes,marketing_consent,is_active,created_at
      ) values (
  ...
  for d in select generate_series(CURRENT_DATE-28,CURRENT_DATE+14,interval '1 day')::date loop
    ...
    -- Elizabeth (Studio Glow). Saturday has only 3 appointments to remain within 09-14.
    for i in 0..(case when dow_i=6 then 2 else 3 end) loop
  ...
  raise notice 'SalonFlow Instagram demo seed complete. Organization: %',org_id;
  raise notice 'Created/updated 4 employees, 4 rooms, 10 services, 4 equipment items, 25 clients and a 43-day appointment window.';
end $$;
