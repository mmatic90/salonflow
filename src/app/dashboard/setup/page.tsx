          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link
                ...
                className="rounded-xl bg-app-accent px-4 py-2 font-semibold text-white transition hover:opacity-90"
              >
                Novi termin
              </Link>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {employees.length === 0 ? (
                ...
                employees.map((employee) => (
                  <div
                    key={employee.id}
                    className="flex items-center gap-3 rounded-xl bg-app-card-alt px-4 py-3"
                  >
                    <span
                      className="h-3 w-3 rounded-full"
                      style={{ backgroundColor: employee.color ?? "#776B5D" }}
                    />
            ...
            <div className="mt-4 space-y-3">
              {services.length === 0 ? (
                ...
                services.map((service) => (
                  <div
                    key={service.id}
                    className="rounded-xl bg-app-card-alt px-4 py-3"
                  >
            ...
            <div className="mt-4 space-y-3">
              {rooms.length === 0 ? (
                ...
                rooms.map((room) => (
                  <div
                    key={room.id}
                    className="rounded-xl bg-app-card-alt px-4 py-3 font-medium text-app-text"
                  >
                    {room.name}
                  </div>
                ))
