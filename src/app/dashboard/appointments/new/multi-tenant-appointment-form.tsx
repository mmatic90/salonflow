...
export default function MultiTenantAppointmentForm({ defaultDate, clients, employees, rooms, services }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error(result.error || "Dostupnost nije moguće provjeriti.");
        ...
        setEmployeeId("");
        setRoomId("");
        setError(requestError instanceof Error ? requestError.message : "Dostupnost nije moguće provjeriti.");
      } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <section className={sectionClass}>
        ...
        <div className="mt-5 grid gap-5 md:grid-cols-2">
          <label className="space-y-2 text-sm font-medium text-app-text">
            ...
            ) : availabilityReady ? <span className="text-xs text-amber-700">Nema slobodnih zaposlenika za odabrani termin.</span> : null}
          </label>
          ...
          <label className="space-y-2 text-sm font-medium text-app-text">
            ...
            ) : availabilityReady && !availabilityPending ? <span className="text-xs text-app-muted">Za ovu uslugu i termin nema slobodne povezane sobe.</span> : null}
          </label>
        </div>
      </section>
              ...
              {filteredClients.length > 0 ? filteredClients.map((client) => (
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 truncate text-xs text-app-muted">{[client.phone, client.email].filter(Boolean).join(" · ") || "Nema spremljenih kontaktnih podataka"}</p>
                  </div>
                </button>
              )) : clientSearch.trim() ? <p className="px-3 py-4 text-sm text-app-muted">Nema spremljenog klijenta koji odgovara pretrazi.</p> : null}
            </div>
          ) : null}
        </div>
      ...
      <section className={sectionClass}>
        <SectionHeader icon={<MessageSquareText className="h-5 w-5" />} title="Napomena" description="Dodajte posebnu želju, važnu informaciju ili internu bilješku." />
